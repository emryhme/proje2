...
router.get('/api/plan', AuthMiddleware.authenticate, AuthMiddleware.requireRole(['OWNER', 'ADMIN', 'MANAGER', 'STAFF']), (req: AuthenticatedRequest, res) => {
  try {
...
router.post('/api/plan/support-requests', AuthMiddleware.authenticate, AuthMiddleware.requireRole(['OWNER', 'ADMIN']), (req: AuthenticatedRequest, res) => {
  try {
    const storeId = req.auth!.storeId;
    ...
    const subscription = db.prepare('SELECT plan_name FROM store_subscriptions WHERE store_id = ?').get(storeId) as any;
    if (!subscription) {
      return res.status(404).json({ success: false, error: 'Mağazanız için aktif plan kaydı bulunamadı.' });
    }

    const openRequest = db.prepare("SELECT id FROM plan_support_requests WHERE store_id = ? AND status = 'open' LIMIT 1").get(storeId) as any;
    if (openRequest) {
    ...
    AuthMiddleware.logAudit(storeId, userId, 'CREATE_PLAN_SUPPORT_REQUEST', 'plan_support_requests', String(result.lastInsertRowid), subscription.plan_name, requestedPlan);

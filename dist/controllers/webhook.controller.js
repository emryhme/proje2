...
class WebhookController {
    ...
    static resolveStore(slug) {
        ...
        if (!cleanSlug)
            return null;
        try {
            const store = db_1.db.prepare('SELECT id, name, slug, status, webhook_verify_token, instagram_account_id FROM stores WHERE LOWER(slug) = ?').get(cleanSlug);
            return store || null;
        }
        catch {
            return null;
    ...
    static extractInstagramComment(change) {
     ...
     */
    static async processCommentAndReply(comment, storeSlug, storeId) {
        try {
            const conversationId = ai_service_1.AIService.getOrCreateConversation(storeId, `instagram:${comment.commenterId}`);
            ai_service_1.AIService.persistMessage(conversationId, 'user', `[Instagram yorumu] ${comment.text}`);
            let aiInput = comment.text;
            if (!(0, regex_util_1.extractProductCode)(comment.text) && comment.mediaId) {
                const mediaContext = await facebook_service_1.FacebookService.getInstagramMediaContext(comment.mediaId, storeId);
                const postProductCode = (0, regex_util_1.extractProductCode)(mediaContext?.caption || '');
                if (postProductCode) {
                    aiInput = `${comment.text}\n\nYorum yapılan gönderideki ürün kodu: ${postProductCode}`;
                }
            }
            const { reply, toolTraces } = await ai_service_1.AIService.processMessage(comment.commenterId, aiInput, storeSlug, storeId);
            ai_service_1.AIService.persistMessage(conversationId, 'assistant', reply);
            for (const trace of toolTraces) {
            ...
            const sent = await facebook_service_1.FacebookService.sendPrivateReplyToComment(comment.commentId, reply, storeId);
            if (!sent) {

...
class DemoAIService {
    ...
    static async reply(message, history = []) {
        const apiKey = String(env_1.env.geminiApiKey || process.env.GEMINI_API_KEY || '').trim();
        if (!apiKey) {
...
`;
        const response = await axios_1.default.post(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${encodeURIComponent(apiKey)}`, {
        ...
        if (!answer)
            throw new Error('Gemini demo yanıtı oluşturamadı.');

...
class AIService {
     ...
     */
    static normalizeSiparisToolInput(input) {
        let data = input;
        ...
        if (command.startsWith('{')) {
            try {
                return { ...data, ...JSON.parse(command) };
            }
            catch { }
        }
        const parsed = {};
    ...
    static getSessionContext(senderId, storeSlug, storeId, channel = 'instagram') {
        ...
        if (typeof ctx.variantVerified !== 'boolean')
...
`;
    ...
    `).all(storeId, shortCode);
        ...
        const containsExactValue = (value) => {
            const escaped = value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    ...
    static createLeafTools(senderId, storeSlug, storeId, channel) {
        ...
        const kayitTool = new tools_1.DynamicTool({
            ...
            `).run(storeId, senderId, rewardCode, loyaltyThreshold);
                        ...
                        const autoRewardNotificationSent = await facebook_service_1.FacebookService.sendMessage(senderId, autoDmText, storeId);
    ...
    static async processMessage(senderId, userMessage, storeSlug, storeId, channel = 'instagram') {
        ...
        try {
            ...
            if (deterministicOrderReply) {
                ctx.history.push(new messages_1.HumanMessage(userMessage), new messages_1.AIMessage(deterministicOrderReply));
